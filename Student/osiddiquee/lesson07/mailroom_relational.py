...
'''
I realize that this next piece of code is impractical.
This is because im basically storing the db in memory which is inefficent
However, it's not like peewee doesn't have it's own reasons for being impractical
Finally, given that this code will be refactored 4 times and given that
I 100% know this database won't ever be large, I have taken an approach to
simplify it's refactoring hurdles
'''

#transforms the db to a db_to_dict
def db_to_dict():
    ...
    query = (Donor
             .select(Donor.donor_name, Donation.donation_amount)
             .join(Donation))

    donor_dict = {}
    for donor, donation in query:
        if donor in donor_dict:
            donor_dict[donor].append(donation)
        else:
            donor_dict[donor] = [donation]

    data = []
    for donor, donations in donor_dict:
        if not data:
            data = [Donor(donor, donations)]
        else:
            data.append(Donor(donor, donations))

    return data


def dict_to_db(donors_database):
    database = SqliteDatabase('mailroom.db')
    ...
    donors = []
    donor_id_lookup = {}
    for id, donor in enumerate(donors_database):
        if not donors:
            [(id, donor.name)]
        else:
            donor.append((id, donor.name))
    donor_id_lookup[donor.name] = id
    ...
    donations = []
    for id, donor in enumerate(donors_database)
        for name, donation in donor.name, donor.donations
            if not donations:
                donations = [(id, donation, donor_id_lookup[name])]
            else:
                donations.append((id, donation, donor_id_lookup[name]))
...
def main():
    #this is the same as the db but provides a starting point
    db = db_to_dict()
