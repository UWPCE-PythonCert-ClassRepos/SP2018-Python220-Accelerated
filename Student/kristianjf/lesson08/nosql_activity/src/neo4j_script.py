...
def run_example_activity():
    ...
    with driver.session() as session:
        ...
        for first, last in [("Alice", "Cooper"),
                            ...
                            ("Marie", "Curie")]:
            cypher = """
              MATCH (n1:Color {color:'red'})
              CREATE (n1)-[favorite_color:FAV_COLOR]->(n2:Person {first_name:'%s', last_name:'%s'})
              RETURN n1
            """ % (first, last)
            session.run(cypher)
        ...
        for first, last in [('Bob', 'Jones'),
                            ...
                            ('Mary', 'Evans')]:
            cypher = """
              MATCH (n1:Color {color:'green'})
              CREATE (n1)-[favorite_color:FAV_COLOR]->(n2:Person {first_name:'%s', last_name:'%s'})
              RETURN n1
            """ % (first, last)
            session.run(cypher)

        log.info("Step 5: Find all person's who favor red color")
        cyph = """
          MATCH (color {color:'red'})
                -[:FAV_COLOR]->(personColors)
          RETURN personColors
          """
        result = session.run(cyph)
        print("Red's favorited by:")
        for rec in result:
            for person in rec.values():
                print(person['first_name'], person['last_name'])

        log.info("Step 5: Find all person's who favor blue color")
        cyph = """
          ...
          """
        result = session.run(cyph)
        print("Blue's favorited by:")
        for rec in result:
            for person in rec.values():
                print(person['first_name'], person['last_name'])

        log.info("Step 5: Find all person's who favor green color")
        cyph = """
